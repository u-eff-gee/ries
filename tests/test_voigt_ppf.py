...
def test_voigt_ppf():
    ...
    pseudo_voigt = PseudoVoigtDistribution(resonance_energy, 1., 1., 100.)

    # Test scalar and array input.
    assert np.isclose(pseudo_voigt.ppf(0.5), resonance_energy, 1e-5)
    assert np.allclose(pseudo_voigt.ppf(np.array([0.5, 0.5])), np.array([resonance_energy, resonance_energy]), 1e-5)
    ...
    with pytest.warns(UserWarning):
        assert pseudo_voigt.ppf(extremely_small_quantile) > cauchy.ppf(extremely_small_quantile)
        assert pseudo_voigt.ppf(extremely_small_quantile) < norm.ppf(extremely_small_quantile)