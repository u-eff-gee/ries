...
class TestIntegration:
    ...
    def test_quad_partition(self, n_points):
        assert np.isclose(
            quad_partition(lambda x: 1.0, np.linspace(0.0, 1.0, n_points))[0],
            1.0,
            rtol=1e-6,
        )
    ...
    def test_nquad_partition(self, n_points):
        assert np.isclose(
            quad_partition(
                lambda x, y: 1.0, np.linspace(0.0, 1.0, n_points), [[0.0, 1.0]]
            )[0],
            1.0,
            rtol=1e-6,
        )
    ...
    def test_darboux(self, n_points):
        ...
        integral = darboux(
            lambda x: 1.0 if np.ndim(x) == 0 else np.ones(len(x)),
            np.linspace(0.0, 1.0, n_points),
        )
        assert np.isclose(integral[0], 1.0, rtol=1e-6)
        assert np.isclose(1.0 + integral[1], 1.0, rtol=1e-6)

        # Call darboux by passing the values of the function at the points x.
        assert np.isclose(
            darboux(np.ones(n_points), np.linspace(0.0, 1.0, n_points))[0],
            1.0,
            rtol=1e-6,
        )
    ...
    def test_resonance_integration(self):
        ...
        cross_section_integral_analytical = (
            cross_section.energy_integrated_cross_section
        )
        cross_section_integral_numerical = quad(cross_section, *limits)[0]

        assert not np.isclose(
            cross_section_integral_numerical,
            cross_section_integral_analytical,
            rtol=1e-1,
        )
    ...
    def test_resonance_integration_2d(self):
        ...
        cross_section_integral_numerical = quad_partition(
            lambda e, z: cross_section(e), energies, [[0.0, 1.0]]
        )[0]
